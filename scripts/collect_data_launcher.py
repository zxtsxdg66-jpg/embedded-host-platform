...
_PRESETS: list[tuple[str, float, float, str, str]] = [
    ...
    (
        ...
        "  为什么低电平用“暂停”而不是“调到 70~80”：报告按原始读数逐点统计\n"
        "  越限段，低电平若贴近 80 会自己抖过阈值，就分不清越限是被你调下来的\n"
        "  还是自己翻转的；暂停后是本底约 38 dB，绝对低于阈值，无歧义。\n"
        "  跑完记得截图：活动日志红字、指标卡红框，以及暂停后颜色自动消失。",
    ),
    (
        "报警阈值抖动（60 秒，可选）——读数贴近阈值时的原始越限翻转",
        60, 5, "报警阈值抖动",
        "让白噪声在 78~83 dB 之间缓慢飘动（手动小幅调音量即可），持续 60 秒。\n"
        "  目的不是演示报警正常，而是看读数贴近阈值时原始越限会翻转多少次。\n"
        "  这正是报警要连续 2 个周期确认、解除另加回差的原因：报告里只持续\n"
        "  1 个点的越限段，界面上不会报警。原始段翻转多不是失败的实验。",
    ),
    (
