"""把"对模型的约束"在真实问答上的样子录下来，供论文插图与答辩展示页使用。

2026-09-23 立。论文第 4 章 4.5 节讲了四层约束与出口五道检查，第 5 章 5.6 节给了统计数字，
但读者一直没见过**一次具体的问答里每一层各做了什么**。本脚本走与正式启动脚本相同的装配
（仿真运行时＋`attach_language_model`＋`poll_once`），逐题记录：
...
1. **现行配置**（采样温度 0.3）：取数、错误前提、注入、征询语气、指令、范围外、复合句各一题，
   问句原文取自 2026-09-14 鲁棒性题库，不重新编写。
2. **较高采样温度 0.8**（即论文表 5-16 的对照条件）：读数类问题各问若干遍，
   目的是录到出口检查**真实拦下**的改写。现行配置下它们处于零触发状态，拦截只在这个条件下出现。

两组的数据源都是仿真模式，读数不是现场值；本脚本要展示的是约束的行为，不是读数本身。

用法::

    python docs/05_Test/baseline/exp_constraint_showcase.py
"""
