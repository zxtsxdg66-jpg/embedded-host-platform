...
| 不可靠的部件 | 怎么处理 |
| --- | --- |
| **串口链路**（会丢字节、会粘包半包） | 长度 + 命令码 + CRC16 的帧格式，字节流拼帧与重同步；链路不保证正确，帧格式保证 |
...
| 自动化测试 | Python **1294** 项 + Android **41** 项，全绿 |
| 静态检查 | `ruff` 无告警，`mypy` 97 个源文件无问题 |
| 问答·拟合题库 | 84 / 87 = 96.6% |
...
本项目为本科毕业设计，独立开发。仓库中不含论文正文与学校模板；
硬件厂商手册、第三方参考资料同样未收录。

**公开时移除的图片资源**（不影响功能，界面外观不变）：

- `src/ui/assets/background_v1.png` —— 旧版背景图
- `src/ui/assets/background_personal.png` —— 个人照片
- `android/.../ic_launcher_photo.webp` —— 第三方插图，2026-09-08 换用矢量图标后即已无引用
- `docs/10_AndroidClient/预览_App图标方案.png`、`预览_首页背景合成效果.png`
  —— 两张设计过程的对照图，整页印着上述第三方插画

界面实际使用的背景（`src/ui/assets/background.png`，17 KB 中性网格）与应用图标
（矢量绘制）均在仓库内，因此本仓库的运行外观与开发时一致。
`docs/` 下若干过程记录写到上述文件"保留未删"，指的是原开发目录的状态。
